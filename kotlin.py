...
f.write(f"class {saveUseCaseClassName}(private val repository: {repositoryClassName}) {{\n")
...
f.write(f"class {fetchUseCaseClassName}(private val repository: {repositoryClassName}) {{\n")
...
f.write(f"class {deleteUseCaseClassName}(private val repository: {repositoryClassName}) {{\n")
f.write(f"fun invoke({paramsDomainName}s: List<{domainName}>): Completable {{ }}\n")
f.write("}\n")
f.close()
...
f.write(f"class {dataRepositoryClassName}(private val remoteDataSource: {remoteDataSourceClassName}, privateval localDataSource: {localDataSourceClassName}): {repositoryClassName} {{\n")
f.write(f"override fun fetch{domainName}s(): Single<List<{domainName}>> {{}}\n")
f.write(f"override fun save{domainName}s({paramsDomainName}s: List<{domainName}>): Completable {{}}\n")
f.write(f"override fun delete{domainName}s({paramsDomainName}s: List<{domainName}>): Completable {{}}\n")
f.write("}\n")
